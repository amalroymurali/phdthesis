...
case_data = {}
for case in cases:
    filename = f"../{case}_richardson_cum1.csv"  # Changed filename
...
for case, data in case_data.items():
    ...
    for caseall, dataall in case_data.items():
        plt.plot(dataall.iloc[:,0], dataall.iloc[:,2],  # Column 2 for richardson_cum2
    ...
    plt.plot(case_22_data.iloc[:,0], case_22_data.iloc[:,2],  # Column 3
    ...
    # Save the plot
    plt.savefig(f'{case}_richardson_cum1.png')
    plt.close()
