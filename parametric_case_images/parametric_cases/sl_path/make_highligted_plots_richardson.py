...
# Non-Noisy case numbers
# 4, 5, 6, 8, 9, 12, 14, 28, 30
...
for case, data in case_data.items():
    ...
    for caseall, dataall in case_data.items():
        plt.semilogy(dataall.iloc[:,1], dataall.iloc[:,2], 
    ...
    plt.ylabel('$Ri_{c}=\\frac{2U}{R}/[\\frac{\\partial U}{\\partial y}+\\frac{U}{R}]$')
    ...
    # Save the plot
    plt.savefig(f'{case}_richardson.png')
    plt.close()
